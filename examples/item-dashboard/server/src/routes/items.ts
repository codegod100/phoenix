...
router.patch('/categories/:id', async (c) => {
  ...
  if (!db.prepare('SELECT id FROM categories WHERE id = ?').get(id)) return c.json({ error: 'Not found' }, 404);
  let body;
  try { body = await c.req.json(); } catch { return c.json({ error: 'Invalid JSON' }, 400); }
...
router.patch('/:id', async (c) => {
  ...
  if (u.category_id !== undefined) {
    if (u.category_id != null && !db.prepare('SELECT id FROM categories WHERE id = ?').get(u.category_id)) {
