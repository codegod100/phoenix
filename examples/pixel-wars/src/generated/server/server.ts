...
const _svcModules = {
  'connection-handling': connectionHandling,
  'rooms': rooms,
};
...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  const requestedPort = port ?? parseInt(process.env.SERVER_PORT ?? process.env.PORT ?? '3001', 10);
...
const isMain = process.argv[1]?.endsWith('/server/server.js') ||
               process.argv[1]?.endsWith('/server/server.ts');
if (isMain) {
  startServer();
}
