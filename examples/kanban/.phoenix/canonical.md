...
node-5f1c8a3e: each url in a multi-line description must be independently linkified (urls separated by newlines must each be clickable)
...
node-9e5b2f8d: scenario multi-url linkification - given card description with two urls, when rendered in column view, then output contains two separate clickable links
node-4c8f1a5b: scenario url linkification after edit - given card with multi-url description is edited and saved, when edit modal closes and dom updates, then all urls remain independently clickable
...
node-3e8a6c2b: deleting column requires at least 2 columns exist (minimum 1 must remain)
...
node-2f6b9c4a: clicking add-column button opens styled modal for column creation
...
node-8c6a4f3e: ui updates must re-render the affected card/column in place
...
node-4c8f3b9e: board fills viewport height without vertical scroll - vertical space is managed by column internals
