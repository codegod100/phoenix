...
const _svcModules = {
  ...
  'deadline-management': deadlineManagement,
...
export function startServer(port?: number): { server: ReturnType<typeof createServer>; port: number; ready: Promise<void> } {
  const requestedPort = port ?? parseInt(process.env.TASKS_PORT ?? process.env.PORT ?? '3001', 10);
  ...
  const ready = new Promise<void>(resolve => {
    server.listen(requestedPort, () => {
      ...
      console.log(`Tasks listening on http://localhost:${actualPort}`);
...
const isMain = process.argv[1]?.endsWith('/tasks/server.js') ||
               process.argv[1]?.endsWith('/tasks/server.ts');
if (isMain) {
  startServer();
}
