...
//!
//! This demonstrates parsing Nickel (NCL) function definitions
...
fn infer_param_type(param: &str, body: &str, _return_type: &str) -> String {
...
pub fn ncl_fn_to_go(ncl_fn: &NclFunction) -> GoFunction {
    let params: Vec<GoParam> = ncl_fn.params.iter().map(|p| {
        let go_type = infer_param_type(p, &ncl_fn.body, &ncl_fn.return_type);
        GoParam {
