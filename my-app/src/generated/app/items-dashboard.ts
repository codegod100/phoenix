...
// No migrations - this module uses the Items API via fetch

// No schemas - this is a web UI module

router.get('/', async (c) => {
  // Fetch categories for filter dropdown
  const categoriesResponse = await fetch(new URL('/categories', c.req.url));
  ...
  const itemsUrl = new URL('/items?' + params.toString(), c.req.url);
  const itemsResponse = await fetch(itemsUrl);
  const items = await itemsResponse.json();
  
  const currentSort = query.sort || 'name';
  const currentOrder = query.order || 'asc';
  const currentSearch = query.search || '';
  const currentCategory = query.category_id || '';
  ...
  <div class="container">
    ...
    <div class="controls">
      ...
      <select id="categoryFilter" class="filter-select">
        ...
        ${categories.map((cat: any) => `<option value="${cat.id}" ${currentCategory == cat.id ? 'selected' : ''}>${cat.name}</option>`).join('')}
      </select>
    ...
    <table>
      <thead>
        <tr>
          <th class="sortable">
            <a href="?${new URLSearchParams({...query, sort: 'name', order: toggleSort('name')}).toString()}">Name ${sortIcon('name')}</a>
          </th>
          <th class="sortable">
            <a href="?${new URLSearchParams({...query, sort: 'quantity', order: toggleSort('quantity')}).toString()}">Quantity ${sortIcon('quantity')}</a>
          </th>
          <th>Category</th>
          <th>Actions</th>
        </tr>
      ...
      <form id="itemForm">
        ...
        <div class="form-group">
          ...
          <select id="itemCategory">
            ...
            ${categories.map((cat: any) => `<option value="${cat.id}">${cat.name}</option>`).join('')}
          </select>
        </div>
