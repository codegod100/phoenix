...
router.patch('/categories/:id', async (c) => {
  ...
  const u = result.data;
  if (u.name !== undefined) {
    db.prepare('UPDATE categories SET name = ? WHERE id = ?').run(u.name, id);
  }
  if (u.description !== undefined) {
    db.prepare('UPDATE categories SET description = ? WHERE id = ?').run(u.description, id);
  }
...
router.get('/', (c) => {
  ...
  if (sort === 'name') {
    ...
    sql += ' ORDER BY items.quantity ASC';
...
router.patch('/:id', async (c) => {
  ...
  if (u.name !== undefined) {
    db.prepare('UPDATE items SET name = ? WHERE id = ?').run(u.name, id);
  }
  if (u.quantity !== undefined) {
    db.prepare('UPDATE items SET quantity = ? WHERE id = ?').run(u.quantity, id);
  }
